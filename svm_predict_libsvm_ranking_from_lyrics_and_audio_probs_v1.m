...
for log2c = -5:2:15,
  ...
  for log2g = -15:2:4,
    ...
    cmd_cross_val = ['-t 2 -v 5 -h 0 -c ', num2str(2^log2c), ' -g ', num2str(2^log2g)];
    ...
    cmd_predict = ['-t 2 -b 1 -h 0 -c ', num2str(2^log2c), ' -g ', num2str(2^log2g)];
...
rand_Yq = zeros(size(Yq_pred_both_interact_scaled,1));
[Yq_pred2, Yq_pred2_accuracy, Yq_pred2_prob_estimates] = svmpredict(Yt, Yq_pred_both_interact_scaled, bestmodel, '-b 1');

final_ranks = get_ranks(Yq_pred2_prob_estimates);
save('-ascii', 'ranks_final.txt','final_ranks');
